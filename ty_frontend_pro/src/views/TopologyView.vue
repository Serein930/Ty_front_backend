...
onMounted(() => {
    ...
    const getRiskText = (risk) => {
        if (risk === 'CRITICAL' || risk === 'HIGH') return '高危';
    ...
    function updateGraphElements() {
        ...
        const nodeEnter = nodeSelection.enter().append("g").attr("class", "node").attr("id", d => `node-${d.id}`).call(d3.drag().on("start", dragstarted).on("drag", dragged).on("end", dragended))
            .on("mousemove", function(e, d) {
                ...
                let name = d.name || d.label || d.value; let isUser = d.type === 'user'; let riskColor = d.risk === 'CRITICAL' || d.risk === 'HIGH' ? 'text-danger' : d.risk === 'MEDIUM' ? 'text-orange-500' : 'text-warning';
                tooltip.innerHTML = `<div class="font-bold ${isUser?'text-white':'text-primary'} mb-1 border-b border-white/20 pb-1 break-all whitespace-normal">${escapeHTML(name)}</div><div class="text-[10px] text-gray-300 leading-tight grid grid-cols-2 gap-1"><span class="truncate" title="本体: ${escapeHTML(d.type)}">本体: ${escapeHTML(d.type)}</span><span class="${riskColor} truncate">风险: ${escapeHTML(getRiskText(d.risk))}</span><span class="col-span-2 text-gray-500 truncate mt-1">介数: ${d.betweennessNorm||0}</span></div>`;
            })
...
<style src="../assets/styles/pages/topology.css"></style>
